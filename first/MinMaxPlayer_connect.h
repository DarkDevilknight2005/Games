...
#ifndef _MINMAXPLAYER_connect_H
#define _MINMAXPLAYER_connect_H
...
private:
    ...
    int calculateMinMax(T s, bool isMaximizing,int alpha,int beta );
...
template <typename T>
int X_O_MinMax_Player<T>::calculateMinMax(T s, bool isMaximizing,int alpha,int beta) {
    ...
    for (int i = 0; i < 6; ++i) {
        for (int j = 0; j < 7; ++j) {
            if (this->boardPtr->update_board(i, j, s)) {
                int value = calculateMinMax(opponentSymbol, !isMaximizing, alpha, beta);
...
std::pair<int, int> X_O_MinMax_Player<T>::getBestMove() {
    ...
    for (int i = 0; i < 6; ++i) {
        for (int j = 0; j < 7; ++j) {
            if (this->boardPtr->update_board(i, j, this->symbol)) {
                int moveValue = calculateMinMax(this->symbol, false, alpha, beta);
                this->boardPtr->update_board(i, j, 0); // Undo move
